...
fn changelog_entries() -> Vec<(&'static str, &'static str)> {
    vec![
        ("v4.0.0", "内置模型目录 120→458 条真实条目（大版本）：① 目录数据改由脚本从 Ollama 官方源自动生成——覆盖 ollama.com/library 全部 243 个模型家族并按参数尺寸档拆分（如 qwen2.5 拆 0.5b/1.5b/3b/7b/14b/32b/72b 各一条），每条体积以 registry manifest 实测为准（324 条实测；registry 不可达时按参数量估算并在数据中标注），原 120 条人工文案一字不改、并补充官方真实下载量；② 新增白盒评分推荐制度——任务匹配(40 分) + 硬件适配(30/22/8/−40) + 官方真实热度对数归一化(0–30，快照并标注采集日期) 三维可解释评分，「推荐评分」成为云库默认排序，「为你选」每个推荐附中文理由，全程可解释、可复现、无任何伪造热度；③ 云库页与索引编辑器改为分页渲染（首屏 60/100 条 + 「显示更多」），搜索防抖扩展到编辑器，千条规模滚动与搜索不卡顿；④ 内置种子改为编译期嵌入 JSON 数据文件（gtk-app/scripts/build_catalog.py 可复现生成），用户叠加层完全向后兼容（新字段缺省解析，老 catalog_user.json 不受影响）；⑤ 诚实边界：目录为离线快照，热度与体积均标注采集口径与来源（registry 实测 / 估算 / 人工录入）"),
